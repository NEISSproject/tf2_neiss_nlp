...
def main(args):
    ...
    if args.wpjson_file:
        ...
        for json_file in json_files:
            assert str(json_file).endswith(
                ".json"
            ), f".lst-list contains a non json file: {json_file}"
    ...
    for i in range(args.parts):
        ...
        partial_lst = data_lst_sorted[
            int(i * split_samples) : int((i + 1) * split_samples)
        ]
        sentence1, sentence2 = " ".join([x[0] for x in partial_lst[0]]), " ".join(
            [x[0] for x in partial_lst[-1]]
        )
        ...
        with open(
            os.path.join(
                parts_out_dir, f"{os.path.basename(out_file_base_name)}_{i}.json"
            ),
            "w",
        ) as json_fp:
        ...
        with open(
            os.path.join(
                parts_out_dir, f"{os.path.basename(out_file_base_name)}_{i}.lst"
            ),
            "w",
        ) as list_fp:
            list_fp.write(
...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help=".tsv_wp.json file to split by length",
    )
    parser.add_argument(
        "--input_list",
        type=str,
        default="",
        help=".lst list file, load all json-entries and split by length",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="tokens",
        help="feature to sort lists: [tokens,]",
    )
    parser.add_argument(
        "--parts", type=int, default=3, help="split into number of parts, default: 3"
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
        default="",
        help="if split by tokens a SubwordTextEncoder is required",
    )
