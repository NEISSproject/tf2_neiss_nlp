...
def main(args):
    if "bert" in os.path.basename(args.model):
    ...
    elif args.pretraining_type != "auto" and os.path.basename(args.model).upper() in [
        "MLM",
        "SOP",
        "NSP",
    ]:
        ...
        raise AttributeError(
            "could not detect pretraining type and it is not set via --pretraining_type"
        )
    # data_params = MLMDataParams()
    # globals()["MLMDataParams"]
    data_params_cls = globals()[pretraining_type + "DataParams"]
    data_params = data_params_cls(
        tokenizer=Resource("resources/tokenizer/tokenizer_de.subwords")
    )
    ...
    if "abs" in os.path.basename(args.model).lower():
        ...
        logging.critical(
            "could not derive positional encoding from model-name, assume rel_pos_enc=True!"
        )
        params.rel_pos_enc = True
    ...
    new_inputs = {
        k: v
        for k, v in bert_model.layers[3].bert.input.items()
        if k in ["seq_length", "text"]
    }
    ...
    if args.out_dir == "":
        ...
        for fd_object in os.listdir(out_dir):
            assert (
                ".pb" not in fd_object
            ), f"out_dir: {out_dir} already contains a *.pb file"
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "--model", type=str, default="", help=".tsv_wp.json file to split by length"
    )
    parser.add_argument(
        "--pretraining_type",
        type=str,
        default="auto",
        help="set pretraining task mlm|nsp|sop if auto detect fails",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
