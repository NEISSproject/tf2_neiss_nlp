...
from tfaip_scenario.nlp.data import NERData, NERDataParams
from tfaip_scenario.nlp import Evaluator
from tfaip.util.argumentparser.parser import add_args_group
...
def main(args):
    ...
    print(args.data)
    ner_data_gen = NERData(args.data)
    ...
    tag_list = [
        ner_data_gen.get_tag_mapper().get_value(x).replace("B-", "")
        for x in range(ner_data_gen.get_tag_mapper().size())
        if "B-" in ner_data_gen.get_tag_mapper().get_value(x)
    ]
