...
class Model(ModelMLM):
    ...
    def _loss(self, inputs, outputs):
        ...
        return {"softmax_cross_entropy": keras.layers.Lambda(loss_fn, name="softmax_cross_entropy")(
            (inputs['tgt_mlm'], outputs['logits_mlm'], inputs['tgt_sop'], outputs['logits_sop']))}
    ...
    def _metric(self) -> Dict[str, SimpleMetric]:
        ...
        metrics["accuracy_sop"] = SimpleMetric('tgt_sop', 'pred_ids_sop', MyAccuracySOP(name="accuracy_sop"))
...
class BERTSOP(GraphBase):
    ...
    def call(self, inputs, **kwargs):
        ...
        graph_out = {"pred_ids_mlm": mlm_pred_ids, 'logits_mlm': mlm_logits,
                     "pred_ids_sop": sop_pred_ids, 'logits_sop': sop_logits}
        return graph_out
...
class MyAccuracySOP(tf.keras.metrics.Accuracy):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        sop_pred_ids = tf.cast(tf.round(tf.reduce_mean(tf.cast(y_pred, tf.float32), axis=-1)), tf.int32)
        super(MyAccuracySOP, self).update_state(y_true=y_true[:, 0],
                                                y_pred=sop_pred_ids,
